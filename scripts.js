...
function createOrganizationCard(org) {
    ...
    const isQuotaFull = org.recruitment.quota && org.recruitment.quota.type === 'limited' && org.applicants >= org.recruitment.quota.number;
    const isDisabled = isExpired || isQuotaFull || org.recruitment.isDisabled || !org.recruitment.isOpen;

    return `
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-gray-200 transition-all duration-200 animate-slide-up cursor-pointer flex flex-col justify-between" 
            ...
            <div class="p-6 flex-grow">
                ...
                <div class="flex items-center justify-between text-sm text-gray-500 mb-4">
                    <span>Kuota Pendaftar: ${org.recruitment.quota ? org.recruitment.quota.display : 'Tanpa Kuota'}</span>
                </div>
...
function performSearch() {
    ...
    if (query) {
        searchResults = searchResults.filter(org => 
            ...
            (org.tags && org.tags.some(tag => tag.toLowerCase().includes(query))) ||
            (org.info && org.info.description && org.info.description.toLowerCase().includes(query))
        );
    }
    ...
    content.innerHTML = `
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-8">
                ...
                ${(org.info.divisions && org.info.divisions.length > 0) || (org.info.generalRequirements && org.info.generalRequirements.length > 0) ? `
                <div>
                    ...
                    ${org.info.divisions && org.info.divisions.length > 0 ? `
                    <div class="grid grid-cols-1 gap-4">
                        ${org.info.divisions.map(div => `
            ...
            <div class="space-y-6">
                <div class="bg-gray-50 rounded-2xl p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">${org.recruitment.deadline ? new Date(org.recruitment.deadline).toLocaleDateString('id-ID') : '-'}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Sisa waktu</span>
                            <span class="font-medium ${isExpired || isDisabled ? 'text-red-600' : 'text-primary-600'}">
                                ${isDisabled ? 'Sudah ditutup' : `${daysLeft} hari lagi`}
                            </span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Kuota</span>
                            <span class="font-medium">${org.recruitment.quota ? org.recruitment.quota.display : 'Tanpa Kuota'}</span>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 pb-6">
                                ...
                                <p class="text-sm text-gray-500 mt-1">
                                    ${phase.startDate ? new Date(phase.startDate).toLocaleDateString('id-ID', {day: 'numeric', month: 'short'}) : '-'}
                                    ${phase.endDate && phase.startDate !== phase.endDate ? ` - ${new Date(phase.endDate).toLocaleDateString('id-ID', {day: 'numeric', month: 'short'})}` : ''}
                                </p>
                            </div>
                        </div>
                        `).join('')}
                ...
                ${org.info.contactInfo && Object.keys(org.info.contactInfo).length > 0 ? `
                <div>
                    ...
                    <div class="space-y-3">
                        ${Object.keys(org.info.contactInfo).map(key => {
                            ...
                            if (key.includes('instagram') || key.includes('Instagram')) {
